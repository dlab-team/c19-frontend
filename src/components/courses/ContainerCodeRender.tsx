...
import type { Problem, Editor, CssCode } from "@/interfaces/problems";
import { handleIA, handleTest } from "@/actions/problems-client-actions";
import Swal from "sweetalert2";
import type { IAResponse } from "../ia/IAMessage";
...
export const ContainerCodeRender = ({ excerciseId, problem }: Props) => {
  ...
  useEffect(() => {
    const initialCssCode = cookieList[excerciseId]
      ? cookieList[excerciseId].css
      : problem.editors.reduce((acc: CssCode, editor: Editor) => {
          if (editor.type === "css") acc[editor.title] = editor.initialCode;
          return acc;
        }, {});
    const initialHTMLcode = cookieList[excerciseId]
      ? cookieList[excerciseId].html
      : problem.editors.find((editor: Editor) => editor.type === "html")
          ?.initialCode || "";
    const initialSolved = cookieList[excerciseId]
      ? cookieList[excerciseId].solved
      : false;
  ...
  const files: Files = problem.editors.reduce((acc: Files, editor: Editor) => {
    acc[editor.title] = {
      name: editor.title,
      language: editor.type,
      value: editor.expectedCode,
      initial: editor.initialCode,
    };
    return acc;
  }, {});
  ...
  const handleClick = async () => {
    ...
    const htmlEditor = problem.editors.find(
      (editor: Editor) => editor.type === "html",
    );
    const cssEditors = problem.editors.filter(
      (editor: Editor) => editor.type === "css",
    );

    const desiredCSSCode: CssCode = cssEditors.reduce(
      (acc: CssCode, editor: Editor) => {
        acc[editor.title] = editor.expectedCode;
        return acc;
      },
      {},
    );
  ...
  const primaryEditor = problem.editors[0];
    ...
    <>
      ...
      >
        ...
        >
          ...
          <div className="d-flex flex-column gap-2">
            <CodeEditor
              type={primaryEditor.type}
              title={primaryEditor.title}
              expectedCode={primaryEditor.expectedCode}
              files={files}
              setHTMLCode={setHTMLCode}
              setCssCodeS={setCssCode}
              stateCssCode={cssCode}
            ...
            <div className="d-flex justify-content-end me-2">
              <button
                className={`${problem.codeType === "html" ? "bg_excercises_html" : "bg_excercises_css"} px-4 py-2 next_button`}
                onClick={() => {
                  handleClick();
                }}
              >
        ...
        >
          ...
          <div>
            <Render
              contenidoHtml={HTMLcode}
              contenidoCss={Object.values(cssCode).join("\n")}
            />
          </div>
        </div>
      </Container>
